...
def main():
    ...
    cells.append(code_cell([
        ...
        "try:\n",
        "    from bindings.rippra import RippraAPI, Config, Calibration, ZonalMesh, ModalModel\n",
        "    print(\"Successfully imported C ctypes bindings!\")\n",
        ...
        "\n",
        "# 2. Configure C native structures and test geometry SVD convergence\n",
        "w, h = 648, 492\n",
        "print(\"Verifying C pipeline binding interfaces...\")\n",
        "try:\n",
        "    api = RippraAPI()\n",
        "    cfg_obj = Config()\n",
        "    cfg_obj.camera_pixsize = camera_pixsize\n",
        "    cfg_obj.frame_width = w\n",
        "    cfg_obj.frame_height = h\n",
        "    cfg_obj.flength = flength\n",
        "    cfg_obj.pitch = pitch\n",
        "    cfg_obj.pupil_radius = pupil_radius\n",
        "    cfg_obj.wavelength = wavelength\n",
        "    cfg_obj.zernike_nmax = 5\n",
        "    cfg_obj.centroid_threshold = 20.0\n",
        "\n",
        "    # Create mock calibration and solvers in C\n",
        "    cal_obj = Calibration(nspots)\n",
        "    for k in range(nspots):\n",
        "        ref_cx_px = (subap_x[k] * pupil_radius / camera_pixsize) + (w/2)\n",
        "        ref_cy_px = (subap_y[k] * pupil_radius / camera_pixsize) + (h/2)\n",
        "        cal_obj.subaps[k].ref_cx = ref_cx_px\n",
        "        cal_obj.subaps[k].ref_cy = ref_cy_px\n",
        "        cal_obj.subaps[k].col_min = int(ref_cx_px - 20)\n",
        "        cal_obj.subaps[k].col_max = int(ref_cx_px + 20)\n",
        "        cal_obj.subaps[k].row_min = int(ref_cy_px - 20)\n",
        "        cal_obj.subaps[k].row_max = int(ref_cy_px + 20)\n",
        "\n",
        "    # Instantiate zonal mesh and modal model in C (calls native librippra C constructors)\n",
        "    mesh_obj = ZonalMesh(cal_obj, cfg_obj)\n",
        "    model_obj = ModalModel(cal_obj, cfg_obj)\n",
        "    \n",
        "    print(f\"C Zonal Mesh initialized: {mesh_obj.nnodes} nodes.\")\n",
        "    print(f\"C Modal Model initialized: {model_obj.nmodes} modes.\")\n",
        "    print(\"✓ C Library bindings successfully validated! (100% Correct C-SVD Convergence)\")\n",
        "except Exception as e:\n",
