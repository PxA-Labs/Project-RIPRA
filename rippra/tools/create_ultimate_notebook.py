...
def main():
    ...
    cells.append(code_cell([
        ...
        "        def __init__(self, input_dim=274, output_dim=20, hidden_dims=[128, 64]):\n",
        "            super().__init__()\n",
        "            layers = []\n",
        "            in_dim = input_dim\n",
        ...
        "            self.fc = nn.Linear(16 * 3 * 3, output_dim)\n",
        "        def forward(self, x):\n",
        "            out = self.conv1(x)\n",
        "            out = self.bn1(out)\n",
        "            out = self.relu(out)\n",
        ...
        "        def __init__(self, input_dim=20, hidden_dim=64, output_dim=20, num_layers=2):\n",
        "            super().__init__()\n",
        ...
        "        def __init__(self, input_dim=274, hidden_dim=64, output_dim=2):\n",
        "            super().__init__()\n",
        "            self.lstm = nn.LSTM(input_dim, hidden_dim, batch_first=True)\n",
        "            self.fc = nn.Linear(hidden_dim, output_dim)\n",
        "        def forward(self, x):\n",
        ...
        "            return self.fc(out[:, -1, :])\n",
        "            \n",
        "    print(\"All PyTorch ML model architectures configured.\")\n",
    ...
    cells.append(code_cell([
        ...
        "        print(f\"⚠️ ONNX export failed: {onnx_err}. (Try running '!pip install onnx onnxscript' in Kaggle to enable exports). Continuing...\")\n",
        "else:\n",
    ...
    cells.append(code_cell([
        ...
        "ax.set_ylabel(\"Reconstruction RMSE (rad)\")\n",
        "ax.set_xticks(np.arange(len(re_methods)))\n",
    ...
    cells.append(code_cell([
        ...
        "    dx_c, dy_c, coeffs_c = ao.process_frame(img_frame, w, h)\n",
        "    print(f\"✓ C-native execution verified successfully! Reconstructed {len(coeffs_c)} modal coefficients.\")\n",
        "except Exception as e:\n",
