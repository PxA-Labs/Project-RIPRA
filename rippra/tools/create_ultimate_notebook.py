...
def main():
    ...
    cells.append(code_cell([
        ...
        "ax.axis('equal')\n",
        "\n",
        "# 6. Airy Disk vs Aberrated PSF (Log scale)\n",
        "ax = axes[5]\n",
        ...
        "psf /= np.max(psf)\n",
        "ax.imshow(np.log10(psf[45:55, 45:55] + 1e-4), cmap='hot')\n",
        "ax.set_title(\"6. Log PSF Profile (Focal Spot)\", fontsize=10, fontweight='bold')\n",
        "ax.axis('off')\n",
        "\n",
        ...
        "ax.grid(True, linestyle='--', alpha=0.5)\n",
        "\n",
        "# 15. Influence Functions of Deformable Mirror Actuator\n",
        "ax = axes[14]\n",
        "act_x = np.linspace(-3, 3, 50)\n",
        "influence = np.exp(-act_x**2 / 1.5) * np.cos(act_x)\n",
        "ax.plot(act_x, influence, 'c-', lw=2)\n",
        "ax.set_title(\"15. DM Influence Function\", fontsize=10, fontweight='bold')\n",
        "ax.set_xlabel(\"Distance (actuator units)\")\n",
        ...
        "ax.grid(True, linestyle='--', alpha=0.5)\n",
        "\n",
        "# 18. Secondary Mirror Spider Obscuration Grid Mask\n",
        "ax = axes[17]\n",
        "ax.scatter(subap_x, subap_y, c='cyan', s=10, label='Valid')\n",
        "# Block out secondary spider support arms (+ shape)\n",
        "ax.axhline(0, color='darkred', lw=4, label='Spider Arm')\n",
        "ax.axvline(0, color='darkred', lw=4)\n",
        "ax.set_title(\"18. Secondary Mirror Spider Mask\", fontsize=10, fontweight='bold')\n",
        "ax.legend(fontsize=8)\n",
        "ax.set_xlim(-1.2, 1.2)\n",
        "ax.set_ylim(-1.2, 1.2)\n",
        "ax.axis('equal')\n",
