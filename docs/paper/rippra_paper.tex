...
A new slope-completion LSTM for partial-signal fallback (Section~\ref{sec:slope-completion}) adds a mean latency of 0.97~ms on CPU (ONNX Runtime), keeping the combined C + ML path under 2~ms and therefore within the AO cycle budget.
...
\subsection{Slope Completion under Partial Occlusion}
\label{sec:slope-completion}

Partial signal loss---caused by fog, scattering, or occlusion---can leave a
fraction of the SH-WFS spots undetected. The pipeline's frame-quality gate
classifies frames as \texttt{OK}, \texttt{DEGRADED}, or \texttt{LOST}. For
\texttt{DEGRADED} frames, a dedicated LSTM completes the missing slopes
before reconstruction.

The model input is the concatenation of zero-filled observed slopes and a
binary mask: $x_t = [\tilde{s}_t; m_t] \in \mathbb{R}^{4N_s}$. The output
is the full current slope vector $\hat{s}_t \in \mathbb{R}^{2N_s}$. It is
trained with a masked reconstruction loss on the missing entries, a
Zernike-projection regulariser, and a temporal-smoothness term. Completed
slopes are constrained by Zernike projection and a per-spot stroke clamp to
$|\Delta x|, |\Delta y| \le \text{pitch}_{px}/2$.

On held-out synthetic sequences with structured masks, the slope-completion
LSTM achieves a masked RMSE of 1.81~px, compared with 2.41~px for spatial
nearest-neighbour interpolation (75\% of the baseline). The ONNX Runtime CPU
latency is 1.14~ms mean / 0.56~ms median, keeping the degraded-frame path
within the 10~ms AO cycle budget. If the model is unavailable the pipeline
falls back to the existing spatial interpolator; \texttt{LOST} frames park
the DM with zero slopes.
...
\item A slope-domain sequence completion model and C ONNX Runtime hook for degraded-frame recovery, with graceful spatial-interpolation fallback and DM-park behaviour for lost frames.
\item Self-contained HTML dashboards for real-time pipeline monitoring and wavefront visualization.
\end{enumerate}

All components operate well within the 10~ms AO cycle budget, with the classical path achieving under 1~ms end-to-end latency and the degraded-frame AI path remaining under 2~ms.
